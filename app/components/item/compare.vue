  ...
  >
    <button class="item-data flex-1 min-w-0 bg-transparent border-none cursor-pointer p-0 rounded-lg flex items-center gap-4 focus:outline-none after:absolute after:inset-0" @click="handleItemClick">
      ...
      <div class="grid gap-1.5 justify-items-start">
        <span class="text-lg text-neutral-50">{{ rate.quote }}</span>
        <span class="text-sm text-neutral-200">{{ getCurrencyName(rate.quote) }}</span>
      </div>

      <div class="ms-auto grid gap-1.5 justify-items-end">
        <span class="text-xl text-neutral-50">{{ exchange.amount ? getExchangeAmount(rate, exchange.amount) : 'Error' }}</span>
